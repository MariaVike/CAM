...
subroutine gw_tend(state, pbuf, dt, ptend, cam_in, flx_heat)
  ...
  real(r8) :: k_eff_tot(state%ncol,pver)
  real(r8) :: var_gwt_tot(state%ncol,pver+1) 
  ...
  real(r8) :: k_eff(state%ncol,pver)
  real(r8) :: var_gwt(state%ncol,pver+1)   ! Variance of T'
  ...
  if (use_gw_convect_dp) then
     ...
     if (use_gw_chem) then 								!MVG
        call effective_gw_diffusivity(ncol, band_mid, wavelength_mid, p, dt, &
             t, rhoi, nm, ni, c, tau, egwdffi, ubi, k_wave, xi, k_e, k_eff,        &
             zm, zi, var_gwt, dtdz, state1%lat(:ncol))
  ...
  if (use_gw_rdg_beta) then
     ...
     IF (use_gw_chem) then                              !MVG
        ...
        rdg_beta_cd_llb, trpd_leewv_rdg_beta,     &
        ptend, flx_heat, k_wave, xi, k_e, k_eff, egwdffi, &
        var_gwt, state1%lat(:ncol), use_gw_chem=use_gw_chem)
...
subroutine gw_rdg_calc( &
   ...
   k_e_orog_tot, k_eff_orog_tot, egwdffi_orog_tot, &
   var_gwt_orog_tot, lat, use_gw_chem) !MVG optional arguments for gw_chem
  ...
  real(r8), intent(out), optional :: k_eff_orog_tot(ncol,pver)
  real(r8), intent(out), optional :: egwdffi_orog_tot(ncol,pver+1)
  real(r8), intent(out), optional :: var_gwt_orog_tot(ncol,pver+1)
 

  real(r8) :: var_gwt_orog(ncol,pver+1)
  ...
  real(r8) :: egwdffi_orog(ncol,pver+1)
   ...
   do nn = 1, n_rdg
     ...
     IF (use_gw_chem) then !MVG
         ...
         call effective_gw_diffusivity(ncol, band_oro, wavelength_mid, p, dt, &
             t, rhoi, nm, ni, c, tau, egwdffi, ubi, k_wave_orog, xi_orog, k_e_orog, k_eff_orog,    &
             zm, zi, var_gwt_orog, dtdz, lat, kwvrdg=kwvrdg)
 	 ...
 	 do k = 1, pver !add up contributions from all ridges
	   ...
	   xi_orog_tot(:,k) = xi_orog_tot(:,k) + xi_orog(:,k)
 	   ...
 	   k_eff_orog_tot(:,k) = k_eff_orog_tot(:,k) + k_eff_orog(:,k)
         enddo
      ...
      if (nn <= 6) then
         ...
         call outfld('k_eff_orog'//cn//'RDG'//trim(type),  k_eff_orog,    ncol, lchnk) 
   ...
   if (use_gw_chem) then !MVG
     ...
     call outfld ('k_eff_orog_tot_'//trim(type), k_eff_orog_tot, ncol, lchnk)
...
subroutine gw_chem_outflds(prefix, lchnk, ncol,  k_wave, &
  ...
  real(r8), intent(in) :: k_eff(ncol,pver)
  ...
  call outfld(trim(prefix)//'_k_eff', k_eff, ncol, lchnk)
